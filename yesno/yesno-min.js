...
var share_lineLink = "http://weipenggame.com";
...
function get_share_descContent()
{
    if(zf.isGameOver)
    {
        if(mainModel.ajaxData != null)
        {
            share_descContent =  mainModel.ajaxData.data.msg + " 快来玩吧! --微朋佳游";
        }
        else
        {
...
//        this.gameoverContext.css("background-size", parseInt(gameoverTitleWidth * 1.1) + "px " + parseInt(gameoverTitleHeight * 3) + "px");
        ...
        this.gameoverBg.height(parseInt(gameoverTitleHeight * 3.8));
        ...
        this.scoreTabel.css("font-size", parseInt(gameoverTitleHeight * 0.7) + "px");
        $("#gameover_chenhao").css("font-size", parseInt(gameoverTitleHeight * 0.9) + "px");
...
//        this.maxScoreTd.css("text-shadow", scoreShadow + "px "+scoreShadow+"px "+scoreShadow+"px #777777");
        ...
        this.btnTabel.css("margin-top", parseInt(gameoverTitleHeight * 0.1) + "px");
        ...
        var btnWidth = (gameoverTitleHeight / 116 ) * 250 * 1.3;
        var btnHeight = gameoverTitleHeight * 1.3;
        ...
        p.flauntBtn.sClick.add(p.onClickFlauntBtn);
    };
    ...
    p.onClickFlauntBtn = function()
    {
        ...
        {
            ajax_share("qzone");
            if(p.isDocumentKeydown)
            {
                p.isDocumentKeydown = false;
                window.open(p.getFlauntUrl("qq"), "_blank");
            }
        }
    };

    p.getFlauntUrl = function(pt)
    {
        return "./php/flaunt.php?score="+mainModel.score+"&head="+mainModel.headEnd+"&pt="+pt+ (mainModel.ajaxData ? "&msg="+mainModel.ajaxData.data.msg : "");
    };
...
//        p.View_hide();
    ...
    p.setData = function()
    {
        ...
        if(!zf.BrowserDetect.isWeixin)
        {
            this.flauntLink.attr("href", this.getFlauntUrl("qzone"));
        }
        else
        {
            this.flauntLink.attr("href", "");
        }
        this.scoreTd.html(mainModel.score);
        this.maxScoreTd.html(mainModel.scoreMax);


        $("#gameover_chenhao").html(mainModel.getChenhao());
